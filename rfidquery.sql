...
SELECT C.hash AS HASH, O.nome AS NOME , A.nivel AS ACESSO FROM CARD AS C LEFT JOIN OPERATOR AS O ON C.idCARD = O.CARD_idCARD INNER JOIN ACCESS AS A ON C.ACCESS_idACCESS = A.idACCESS WHERE C.STATUS = 'A';call arduino.search_and_grant('574abbbe8a76c77833b471fdaaa6bfb','Arduino');
call arduino.search_and_grant('574abbbe8a76c77833b471fdaaa6bfb','Arduino');
call arduino.search_and_grant('574abbbe8a76c77833b471fdaaa6bfb','Arduino');

SELECT STR_TO_DATE(data,'%d.%m.%y') FROM arduino.LOGS_ACESS L WHERE idLOG = 1; 
SELECT DATE_FORMAT(data, '%d/%m/%Y') FROM arduino.LOGS_ACESS L WHERE idLOG = 1; 


SELECT * FROM arduino.LOGS_ACESS WHERE data < 01/01/2001

SELECT L.idLOG AS ID, L.hash AS HASH, L.data AS DATA, IFNULL(O.nome, 'DESCONHECIDO') AS OPERADOR, CASE L.granted WHEN 0 THEN 'NÃO GARANTIDO' ELSE 'GARANTIDO' END AS GRANTED, R.nome_sala AS SALA FROM arduino.LOGS_ACESS AS L LEFT JOIN arduino.CARD AS C ON L.CARD_idCARD = C.idCARD INNER JOIN arduino.ROOM AS R ON L.ROOM_idROOM = R.idROOM LEFT JOIN arduino.OPERATOR AS O ON L.OPERATOR_idUSER = O.idUSER WHERE L.DATA >= DATE('2018/12/02');