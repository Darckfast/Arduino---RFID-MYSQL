...
SELECT STR_TO_DATE('01/01/2001','%d.%m.%y'); 
SELECT DATE_FORMAT('01/01/2001', '%d/%m/%Y'); 


SELECT * FROM arduino.LOGS_ACESS WHERE data < 01/01/2001

SELECT L.idLOG AS ID, L.hash AS HASH, L.data AS DATA, IFNULL(O.nome, 'DESCONHECIDO') AS OPERADOR, CASE L.granted WHEN 0 THEN 'NÃO GARANTIDO' ELSE 'GARANTIDO' END AS GRANTED, R.nome_sala AS SALA FROM arduino.LOGS_ACESS AS L LEFT JOIN arduino.CARD AS C ON L.CARD_idCARD = C.idCARD INNER JOIN arduino.ROOM AS R ON L.ROOM_idROOM = R.idROOM LEFT JOIN arduino.OPERATOR AS O ON L.OPERATOR_idUSER = O.idUSER WHERE L.DATA >= DATE('2000/01/01') AND L.DATA <= DATE('2018/12/02');